...
void SP2::Update(double dt)
{
	...
	else if (m_bMenu == false)
	{
		...
		if (Application::IsKeyPressed('H') && m_dBounceTime <= 0)
		{
			...
			//world[int((camera.position.x + scale* 0.5f) / scale)][int((camera.position.z + scale* 0.5f) / scale)] = 'F';

		/*	world[125][125] = 'D';

			world[int((camera.position.x + scale* 0.5f) / scale)][int((camera.position.z + scale* 0.5f) / scale)] = 'c';*/
	...
	if (player->GetPlaceDown())
	{
		if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_FURNACE)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(y) + "F");
			}
			...
			player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_HOE)
		{
			...
			if (world[x][y] == 'G')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(y) + "t");
				}
				world[x][y] = 't';
			}
		...
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_SEED)
		{
			...
			if (world[x][y] == 't')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(y) + "w");
				}
		...
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_CARROT)
		{
			...
			if (world[x][y] == 't')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(y) + "c");
				}
	...
	{
		int x = static_cast<int>((player->getcurtool()->GetBlockPlacement().x + scale* 0.5f) / scale);
		int z = static_cast<int>((player->getcurtool()->GetBlockPlacement().z + scale* 0.5f) / scale);
		...
		if (world[x][z] == 'c') // Furnace
		{
			...
			for (unsigned int i = 0; i < CropList.size(); ++i)
			{
				if (x == CropList[i]->GetXTile() && z == CropList[i]->GetZTile())
				{
					...
					for (int i = 0; i < 10; i++)
					{
						MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(z) + "R");
					}
		...
		player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_PICKAXE) 
		{
			...
			if (world[x][z] == 'F') // Furnace
			{
				...
				if (m_swingcount > player->getcurtool()->GetIntMaxSwings())
				{
					...
					for (int i = 0; i < 10; i++)
					{
						MESSAGES_WORLDITEM.push_back("2@" + std::to_string(x) + "," + std::to_string(z) + "G");
					}
					player->addItem(new Item(Item::ITEM_FURNACE, 1));
			...
			else if (world[x][z] == 'C') // Coal
			{
				...
				if (m_swingcount > player->getcurtool()->GetIntMaxSwings())
				{
					...
					player->addItem(new Item(Item::ITEM_STONE, randVal));
	...
	// Player movemetn across client to server 
	if (Application::IsKeyPressed('W'))
	{
		MESSAGES.push_back("2W" + std::to_string(camera.position.x).substr(0, 5)
			+ ":" + std::to_string(camera.position.z).substr(0, 5) + "^" + std::to_string(packetID++));

	}
	if (Application::IsKeyPressed('A'))
	{
		MESSAGES.push_back("2A" + std::to_string(camera.position.x).substr(0, 5)
			+ ":" + std::to_string(camera.position.z).substr(0, 5) + "^" + std::to_string(packetID++));
	}

	if (Application::IsKeyPressed('S'))
	{
		MESSAGES.push_back("2S" + std::to_string(camera.position.x).substr(0, 5)
			+ ":" + std::to_string(camera.position.z).substr(0, 5) + "^" + std::to_string(packetID++));
	}

	if (Application::IsKeyPressed('D'))
	{
		MESSAGES.push_back("2D" + std::to_string(camera.position.x).substr(0, 5)
			+ ":" + std::to_string(camera.position.z).substr(0, 5) + "^" + std::to_string(packetID++));
	}
	/*
		Ones i get the packet then send it to the player pos int the 2 player
	*/
	if (MESSAGES_USE.size() != 0)
	{
		for (unsigned int i = 0; i < MESSAGES_USE.size(); i++)
		{
			//std::cout << MESSAGES_USE[i] << std::endl;
			int q;

			if (MESSAGES_USE[i].substr(2, 1) == "W")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = stof(MESSAGES_USE[i].substr(3, 5));
				Zpos = stof(MESSAGES_USE[i].substr(q + 1, 5));

				//std::cout << Xpos << ":" << Zpos << std::endl;

			}
			if (MESSAGES_USE[i].substr(2, 1) == "A")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = stof(MESSAGES_USE[i].substr(3, 5));
				Zpos = stof(MESSAGES_USE[i].substr(q + 1, 5));
			}
			if (MESSAGES_USE[i].substr(2, 1) == "S")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = stof(MESSAGES_USE[i].substr(3, 5));
				Zpos = stof(MESSAGES_USE[i].substr(q + 1, 5));
			}
			if (MESSAGES_USE[i].substr(2, 1) == "D")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = stof(MESSAGES_USE[i].substr(3, 5));
				Zpos = stof(MESSAGES_USE[i].substr(q + 1, 5));
			}
			if (MESSAGES_USE[i].substr(1, 1) == "I")
			{
...
void SP2::SeasonChanger(double dt)
{
	...
	if (static_cast<int>(SP2_Seasons->getTimer()) == -1)
	{
		m_bTexChange = false;
	}
...
ProjectileObject* SP2::GetProjectile(void)
{
	...
	for (unsigned int i = 0; i < static_cast<unsigned int>(MAX_PROJECTILE); ++i)
	{
...
void SP2::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	...
	modelStack.Translate(Application::GetWindowWidth()* 0.5f + x ,Application::GetWindowHeight()* 0.5f + y, 0);
...
void SP2::RenderCrafting()
{
	...
	if (player->getIsCrafting() == true)
	{
		...
		RenderImageToScreen(meshList[GEO_CRAFTING_MENU], false, Application::GetWindowWidth()* 0.5f, Application::GetWindowWidth()* 0.5f,
			Application::GetWindowWidth()* 0.5f, Application::GetWindowHeight()* 0.5f, 0);
...
void SP2::RenderPassMain()
{
	...
	RenderInstructions();
			...
			ss.str("");
			ss << std::to_string(int((camera.position.x + scale* 0.5f) / scale)) << " " << std::to_string(int((camera.position.z + scale* 0.5f) / scale));
