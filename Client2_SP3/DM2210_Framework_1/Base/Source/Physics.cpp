...
{ 
	...
	for (int i = 0; i < 4; i++)
	{
		...
		Projection p1(static_cast<float>(min), static_cast<float>(max));
		...
		Projection p2(static_cast<float>(min), static_cast<float>(max));
		if (p1.max > p2.min)
		{
			return false;
		}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		Projection p2(static_cast<float>(min), static_cast<float>(max));
		if (static_cast<float>(p2.max) > static_cast<float>(p1.min))
		{
			return false;
		}
		if (static_cast<float>(p2.min) > static_cast<float>(p1.max))
		{
			return false;
		}
	}
