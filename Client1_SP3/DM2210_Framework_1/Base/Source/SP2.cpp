...
void SP2::ListToServer() {
	...
	while (true)
	{
		...
		iWsaStartup = WSAStartup(MAKEWORD(2, 2), &WinSockData); // NEW
		UDPServer.sin_family = AF_INET;// NEW
		UDPServer.sin_addr.s_addr = inet_addr(getIPv4().c_str());// NEW
		UDPServer.sin_port = htons(8001);// NEW
		UDPSocketClient = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
		...
		std::string sms = Buffer;

		if (sms.substr(0, 1) == "!")
		{
			for (unsigned int i = 0; i < MESSAGES_WORLDITEM.size(); i++)
			{
				TalkToServer(MESSAGES_WORLDITEM[i]);
				MESSAGES_WORLDITEM.erase(MESSAGES_WORLDITEM.begin() + i);
			}

			for (unsigned int i = 0; i < MESSAGES.size(); i++)
			{
				TalkToServer(MESSAGES[i]);
				MESSAGES.erase(MESSAGES.begin() + i);
			}
		}
		else
		{
			MESSAGES_USE.push_back(sms);
		}
...
void SP2::Init()
{
	...
	m_fAmbient = 0.6f;
	minOutwardsFromPlayerX = static_cast<int>(pX - outwards);
	minOutwardsFromPlayerZ = static_cast<int>(pZ - outwards);

	maxOutwardsFromPlayerX = static_cast<int> (pX + outwards);
	maxOutwardsFromPlayerZ = static_cast<int> (pZ + outwards);
	...
	meshList[GEO_GOLD_SWORD_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_SWORD_MODEL", "OBJ//sword.obj");
	meshList[GEO_GOLD_SWORD_MODEL]->textureArray[0] = LoadTGA("Image//Gold_Sword.tga");

	meshList[GEO_WOODEN_AXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_WOODEN_AXE_MODEL", "OBJ//axe.obj");
	meshList[GEO_WOODEN_AXE_MODEL]->textureArray[0] = LoadTGA("Image//Wooden_Axe.tga");

	meshList[GEO_STONE_AXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_AXE_MODEL", "OBJ//axe.obj");
	meshList[GEO_STONE_AXE_MODEL]->textureArray[0] = LoadTGA("Image//Stone_Axe.tga");

	meshList[GEO_GOLD_AXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_AXE_MODEL", "OBJ//axe.obj");
	meshList[GEO_GOLD_AXE_MODEL]->textureArray[0] = LoadTGA("Image//Gold_Axe.tga");

	meshList[GEO_WOODEN_HOE_MODEL] = MeshBuilder::GenerateOBJ("GEO_WOODEN_HOE_MODEL", "OBJ//hoe.obj");
	meshList[GEO_WOODEN_HOE_MODEL]->textureArray[0] = LoadTGA("Image//Wooden_Hoe.tga");

	meshList[GEO_STONE_HOE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_HOE_MODEL", "OBJ//hoe.obj");
	meshList[GEO_STONE_HOE_MODEL]->textureArray[0] = LoadTGA("Image//Stone_Hoe.tga");

	meshList[GEO_GOLD_HOE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_HOE_MODEL", "OBJ//hoe.obj");
	meshList[GEO_GOLD_HOE_MODEL]->textureArray[0] = LoadTGA("Image//Gold_Hoe.tga");

	meshList[GEO_WOODEN_PICKAXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_WOODEN_PICKAXE_MODEL", "OBJ//pickaxe.obj");
	meshList[GEO_WOODEN_PICKAXE_MODEL]->textureArray[0] = LoadTGA("Image//Wooden_Pickaxe.tga");

	meshList[GEO_STONE_PICKAXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_PICKAXE_MODEL", "OBJ//pickaxe.obj");
	meshList[GEO_STONE_PICKAXE_MODEL]->textureArray[0] = LoadTGA("Image//Stone_Pickaxe.tga");

	meshList[GEO_GOLD_PICKAXE_MODEL] = MeshBuilder::GenerateOBJ("GEO_STONE_PICKAXE_MODEL", "OBJ//pickaxe.obj");
	meshList[GEO_GOLD_PICKAXE_MODEL]->textureArray[0] = LoadTGA("Image//Gold_Pickaxe.tga");
	//wORLD
	...
	Thread = std::thread([this] { this->ListToServer(); });
	Thread.detach();
...
void SP2::RenderFurnace()
{
	for (unsigned int i = 0; i < FurnaceList.size(); ++i)
	{
		...
		{
			//Interface
			RenderImageToScreen(meshList[GEO_SMELTING_MENU], false, Application::GetWindowWidth()* 0.5f, Application::GetWindowWidth()* 0.5f,
				Application::GetWindowWidth()* 0.5f, Application::GetWindowHeight()* 0.5f, 0);

			//Top
			RenderImageToScreen(meshList[GEO_EMPTY_CRAFTING], false, 175, 175,
				Application::GetWindowWidth()* 0.5f - 150, Application::GetWindowHeight()* 0.5f + 150, 1);

			RenderItem(Application::GetWindowWidth()* 0.5f - 150, Application::GetWindowHeight()* 0.5f + 150
			...
			//Result
			RenderImageToScreen(meshList[GEO_EMPTY_CRAFTING], false, 175, 175,
				Application::GetWindowWidth()* 0.5f + 150, Application::GetWindowHeight()* 0.5f, 1);

			RenderItem(Application::GetWindowWidth()* 0.5f + 150, Application::GetWindowHeight()* 0.5f
			...
			if (FurnaceList[i]->GetSlot() == 1)
				RenderImageToScreen(meshList[GEO_HIGHLIGHT_INVENTORY], false, 175, 175,
					Application::GetWindowWidth()* 0.5f - 150, Application::GetWindowHeight()* 0.5f + 150, 2);
			else if (FurnaceList[i]->GetSlot() == 2)
				RenderImageToScreen(meshList[GEO_HIGHLIGHT_INVENTORY], false, 175, 175,
					Application::GetWindowWidth()* 0.5f - 150, Application::GetWindowHeight()* 0.5f - 150, 2);
			else
				RenderImageToScreen(meshList[GEO_HIGHLIGHT_INVENTORY], false, 175, 175,
					Application::GetWindowWidth()* 0.5f + 150, Application::GetWindowHeight()* 0.5f, 2);
...
{
	int x = static_cast<int>((xPos + scale * 0.5f) / scale);
	int y = static_cast<int>((yPos + scale * 0.5f) / scale);
	return world[x][y];
}
...
{
	m_dBounceTime -= 1 * dt;
	m_fConstantRotate += 20 * static_cast<float>(dt);
	...
	else if (m_bMenu == false)
	{
		...
		{
			int x = static_cast<int>(camera.position.x / scale);
			int z = static_cast<int>(camera.position.z / scale);
		}
		...
		if (m_fTimeTillLightning <= 0.2)
		{
			...
			m_fTimeTillLightning += 0.1f *  static_cast<float>(dt);
		}
		else
		{
			lights[0].power = 1;
			glUniform1f(m_parameters[U_LIGHT0_POWER], lights[0].power);
			m_fTimeTillLightning = -0.3f;
			m_bRandLightning = true;
		}
		...
		if (Application::IsKeyPressed('H') && m_dBounceTime <= 0)
		{
			...
			//world[int((camera.position.x + scale * 0.5) / scale)][int((camera.position.z + scale * 0.5) / scale)] = 'F';

		/*	world[125][125] = 'D';

			world[int((camera.position.x + scale * 0.5) / scale)][int((camera.position.z + scale * 0.5) / scale)] = 'c';*/
		...
		{
			m_fAmbient -= 0.002f *  static_cast<float>(dt);
		...
		else if (m_iDayNight == -1)
		{
			...
			m_fDayNightDuration -= 1 * static_cast<float>(dt);
	...
	{
		int x = static_cast<int>((player->getcurtool()->GetBlockPlacement().x + scale * 0.5f) / scale);
		int y = static_cast<int>((player->getcurtool()->GetBlockPlacement().z + scale * 0.5f) / scale);
		world[x][y] = 'b';
	...
	if (player->GetPlaceDown())
	{
		if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_FURNACE)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				MESSAGES_WORLDITEM.push_back("1I" + std::to_string(x) + "," + std::to_string(y) + "F");
			}
			...
			player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_HOE)
		{
			...
			if (world[x][y] == 'G')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("1I" + std::to_string(x) + "," + std::to_string(y) + "t");
				}
			}
		...
		{
			int x = static_cast<int>((player->getcurtool()->GetBlockPlacement().x + scale * 0.5) / scale);
			int y = static_cast<int>((player->getcurtool()->GetBlockPlacement().z + scale * 0.5) / scale);
			...
			if (world[x][y] == 't')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("1I" + std::to_string(x) + "," + std::to_string(y) + "w");
				}
		...
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_CARROT)
		{
			...
			if (world[x][y] == 't')
			{
				...
				for (int i = 0; i < 10; i++)
				{
					MESSAGES_WORLDITEM.push_back("1I" + std::to_string(x) + "," + std::to_string(y) + "c");
				}
			}
		}
	}
	...
	{
		int x = static_cast<int>((player->getcurtool()->GetBlockPlacement().x + scale * 0.5) / scale);
		int z = static_cast<int>((player->getcurtool()->GetBlockPlacement().z + scale * 0.5) / scale);

		if (world[x][z] == 'c') // Carrot
		{
			world[x][z] = 't';
			...
			for (unsigned int i = 0; i < CropList.size(); ++i)
			{
				if (x == CropList[i]->GetXTile() && z == CropList[i]->GetZTile())
				{
					...
					for (int i = 0; i < 10; i++)
					{
						MESSAGES_WORLDITEM.push_back("1I" + std::to_string(x) + "," + std::to_string(z) + "R");
					}
		...
		player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_PICKAXE) 
		{
				...
				if (m_swingcount > player->getcurtool()->GetIntMaxSwings())
				{
					...
					for (int i = 0; i < FurnaceList.size(); ++i)
					{
						if (FurnaceList[i]->GetXTile() == x && FurnaceList[i]->GetZTile() == z)
						{
							...
							FurnaceList.erase(FurnaceList.begin() + i);
							break;
						}
					}
					m_swingcount = 0;
			...
			else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_PICKAXE ||
						player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_PICKAXE)// Gold Ore
			{
				if (world[x][z] == 'O')
				{
					++m_swingcount;
		...
		instructiontimer += static_cast<float>(dt);
	...
	std::cout << world[(int((camera.position.x + scale * 0.5) / scale))][(int((camera.position.z + scale * 0.5) / scale))] << std::endl;


	if (MESSAGES_USE.size() != 0)
	{
		for (unsigned int i = 0; i < MESSAGES_USE.size() - 1; i++)
		{
			std::cout << MESSAGES_USE[i] << std::endl;

			int q;

			if (MESSAGES_USE[i].substr(1, 1) == "W")
			{

				q = MESSAGES_USE[i].find(":");
				Xpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(2, 5)));
				Zpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(q + 1, 5)));


			}
			if (MESSAGES_USE[i].substr(1, 1) == "A")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(2, 5)));
				Zpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(q + 1, 5)));
			...
			if (MESSAGES_USE[i].substr(1, 1) == "S")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(2, 5)));
				Zpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(q + 1, 5)));
			}
			if (MESSAGES_USE[i].substr(1, 1) == "D")
			{
				q = MESSAGES_USE[i].find(":");
				Xpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(2, 5)));
				Zpos = static_cast<float>(stoi(MESSAGES_USE[i].substr(q + 1, 5)));
			...
			if (MESSAGES_USE[i].substr(1, 1) == "@")
			{
				//std::cout << "got it " << std::endl;

				int findAnd = MESSAGES_USE[i].find(",");
				int xpos = stoi(MESSAGES_USE[i].substr(2, findAnd - 2));
				int zpos = stoi(MESSAGES_USE[i].substr(findAnd + 1, MESSAGES_USE[i].size() - findAnd - 2));
				item = MESSAGES_USE[i].substr(MESSAGES_USE[i].size() - 1);
				...
				if (item == "R")
				{
					for (unsigned int i = 0; i < CropList.size(); ++i)
					{
						if (xpos == CropList[i]->GetXTile() && zpos == CropList[i]->GetZTile())
						{
							world[xpos][zpos] = 't';
							CropList.erase(CropList.begin() + i);
							break;
				...
				{
					CropList.push_back(new Crops(0, xpos, zpos, 0));

					cstr[item.size() + 1];
					item.copy(cstr, item.size() + 1);
					cstr[item.size()] = '\0';
					world[xpos][zpos] = cstr[0];
					
				}
				else if (item == "w")
				{
					CropList.push_back(new Crops(1, xpos, zpos, 0));
			...
			MESSAGES_USE.erase(MESSAGES_USE.begin() + i);
		}
	}
	...
	{
		MESSAGES.push_back(IPtoASCII() + "W" + std::to_string(camera.position.x).substr(0, 5) + ":" + std::to_string(camera.position.z).substr(0, 5) + "&" + std::to_string(packetID++));
	}

	if (Application::IsKeyPressed('S'))
	{
		MESSAGES.push_back(IPtoASCII() + "S" + std::to_string(camera.position.x).substr(0, 5) + ":" + std::to_string(camera.position.z).substr(0, 5) + "&" + std::to_string(packetID++));
	}

	if (Application::IsKeyPressed('A'))
	{
		MESSAGES.push_back(IPtoASCII() + "A" + std::to_string(camera.position.x).substr(0, 5) + ":" + std::to_string(camera.position.z).substr(0, 5) + "&" + std::to_string(packetID++));
	}

	if (Application::IsKeyPressed('D'))
	{
		MESSAGES.push_back(IPtoASCII() + "D" + std::to_string(camera.position.x).substr(0, 5) + ":" + std::to_string(camera.position.z).substr(0, 5) + "&" + std::to_string(packetID++));
	}
		...
		if (i >= 0 && i <= 250)
		{
			for (int k = minOutwardsFromPlayerZ; k < maxOutwardsFromPlayerZ; ++k)
			{
				...
				if (k >= 0 && k <= 250)
				{
					...
					{
						int choice = Math::RandIntMinMax(0, 20);
						if (m_NumOfEnemy < 10 && choice == 0 && (Vector3(0 + i * scale, 0, 0 + k * scale) - camera.position).Length() > 100) //spawn in if it is 1
						{
							CEnemy *go = EnemyFetchGO();
			...
			if (i >= 0 && i <= 250)
			{
				for (int k = minOutwardsFromPlayerZ; k < maxOutwardsFromPlayerZ; ++k)
				{
					...
					if (k >= 0 && k <= 250)
					{
						if (world[i][k] == 'G')
						{
							int choice = Math::RandIntMinMax(0, 20 + 5 * static_cast<int>(dt));
...
void SP2::UpdateParticles(double dt)
{
	...
	m_fWindBounceTime += 2 * static_cast<float>(dt);
...
void SP2::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	...
	modelStack.Translate(static_cast<float>(Application::GetWindowWidth() * 0.5 + x) , static_cast<float>(Application::GetWindowHeight() * 0.5 + y), 0);
...
void SP2::RenderCrafting()
{
	...
	if (player->getIsCrafting() == true)
	{
		...
		RenderImageToScreen(meshList[GEO_CRAFTING_MENU], false, static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowWidth()* 0.5f),
			static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f), 0);

		//Left Slot
		RenderImageToScreen(meshList[GEO_EMPTY_CRAFTING], false, 175.f, 175.f,
			static_cast<float>(Application::GetWindowWidth()* 0.5f - 150), static_cast<float>(Application::GetWindowHeight()* 0.5f), 1.f);

		if (player->getCraftingSlotOne() != -1)
			//Render the id of whatever belongs to the first crafting slot
			RenderItem(static_cast<float>(Application::GetWindowWidth()* 0.5f - 150), static_cast<float>(Application::GetWindowHeight()* 0.5f)
				, 2.f, 150.f, 150.f, player->getItem(player->getCraftingSlotOne())->getID());
		//
		...
		if (player->getCraftingSlotTwo() != -1)
			...
			RenderItem(static_cast<float>(Application::GetWindowWidth()* 0.5f + 150), static_cast<float>(Application::GetWindowHeight()* 0.5f)
				, 2.f, 150.f, 150.f, player->getItem(player->getCraftingSlotTwo())->getID());
		//

		//Bottom Crafting
		RenderImageToScreen(meshList[GEO_EMPTY_CRAFTING], false, 175.f, 175.f,
			static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f - 200), 1.f);

		if (player->getCraftingSlotTwo() != -1)
			RenderItem(static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f - 200)
				, 2.f, 150.f, 150.f, player->craft(player->getCraftingSlotOne(), player->getCraftingSlotTwo())->getID());
		//+
	}
}
...
{
	RenderImageToScreen(meshList[GEO_INVENTORY], false, static_cast<float>(Application::GetWindowWidth()), static_cast<float>(Application::GetWindowHeight() / 10),
		static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f - 360), 0);

	for (unsigned int i = 0; i < static_cast<unsigned int>(player->getTotalItems()); ++i)
	{
		//Frames for inventory
		if (i != player->getCurrentSlot())
			RenderImageToScreen(meshList[GEO_EMPTY_INVENTORY], false, 60, 60,
				180.f + 60.f + i * 60.f, static_cast<float>(Application::GetWindowHeight()* 0.5f - 360), 1);
		else
			RenderImageToScreen(meshList[GEO_HIGHLIGHT_INVENTORY], false, 60, 60,
				180.f + 60.f + i * 60.f, static_cast<float>(Application::GetWindowHeight()* 0.5f - 360), 1);

		RenderItem(180.f + 60.f + i * 60.f, static_cast<float>(Application::GetWindowHeight()* 0.5f - 360), 2.f, 50.f, 50.f, player->getItem(i)->getID());
	}
}
void SP2::RenderPlayerInfo()
{
	//Health
	for (unsigned int i = 0; i < static_cast<float>(player->getMaxHP()) * 0.1f; ++i)
	{
		if (i < static_cast<unsigned int>(player->getHP() * 0.1))
		{
			RenderImageToScreen(meshList[GEO_HEALTH_FULL], false, 40.f, 40.f,
				120.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 20), 0);
		}
		else
		{
			if (player->getHP() > 0 && static_cast<int>(player->getHP()) % 2 == 1 && i * 10 < static_cast<unsigned int>(player->getHP()))
				RenderImageToScreen(meshList[GEO_HEALTH_HALF], false, 40.f, 40.f,
					120.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 20), 0);
			else
				RenderImageToScreen(meshList[GEO_HEALTH_EMPTY], false, 40.f, 40.f,
					120.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 20), 0);
		}
	}
	//Hunger
	...
	{
		if (i < static_cast<unsigned int>(player->getHunger() * 0.1f))
		{
			RenderImageToScreen(meshList[GEO_HUNGER_FULL], false, 40.f, 40.f,
				650.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 40), 0);
		}
		else
		{
			if (player->getThirst() > 0 && static_cast<int>(player->getHunger()) % 2 == 1 && i * 10 < static_cast<unsigned int>(player->getHunger()))
				RenderImageToScreen(meshList[GEO_HUNGER_HALF], false, 40.f, 40.f,
					650.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 40), 0);
			else
				RenderImageToScreen(meshList[GEO_HUNGER_EMPTY], false, 40.f, 40.f,
					650.f + 40.f + i * 40.f, static_cast<float>(Application::GetWindowHeight() * 0.2f - 40), 0);
		}
	}
	//Thirst
	for (unsigned int i = 0; i < 10; ++i)
	{
		if (player->getThirst() > 0 && i < static_cast<unsigned int>(player->getThirst() * 0.1f))
		{
			RenderImageToScreen(meshList[GEO_THIRST_FULL], false, 40.f, 40.f,
				static_cast<float>(650 + 40 + i * 40), static_cast<float>(Application::GetWindowHeight() * 0.2f + 10), 0);
		}
		else
		{
			RenderImageToScreen(meshList[GEO_THIRST_EMPTY], false, 40.f, 40.f,
				static_cast<float>(650 + 40 + i * 40), static_cast<float>(Application::GetWindowHeight() * 0.2f + 10), 0.f);
		}
	}
}
void SP2::RenderWorld()
{
	...
	if (m_bMenu == false)
	{
		...
		for (int i = 0; i < 15; ++i)
		{
			...
			{
				modelStack.PushMatrix();
				modelStack.Translate(lightningX + 50.f, static_cast<float>(1440 - i * 96), lightningZ);
...
{
	if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_SWORD || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_SWORD || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_SWORD)
	{
		...
		{
			RenderMesh(meshList[GEO_WOODEN_SWORD_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_SWORD)
		{
			RenderMesh(meshList[GEO_STONE_SWORD_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_SWORD)
		{
			RenderMesh(meshList[GEO_GOLD_SWORD_MODEL], false);
		}
		modelStack.PopMatrix();
		modelStack.PopMatrix();
		//
	...
	if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_AXE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_AXE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_AXE)
	{
		Vector3 weaponoffset(1, 1, 2);
		Vector3 viewvector = (camera.target - camera.position);
		modelStack.PushMatrix();
		...
		modelStack.Scale(2, 2, 2);
		if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_AXE)
		{
			RenderMesh(meshList[GEO_WOODEN_AXE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_AXE)
		{
			RenderMesh(meshList[GEO_STONE_AXE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_AXE)
		{
			RenderMesh(meshList[GEO_GOLD_AXE_MODEL], false);
		}
		modelStack.PopMatrix();
		modelStack.PopMatrix();
		//
	...
	if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_HOE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_HOE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_HOE)
	{
		Vector3 weaponoffset(1, 1, 2);
		Vector3 viewvector = (camera.target - camera.position);
		modelStack.PushMatrix();
		...
		modelStack.Scale(2, 2, 2);
		if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_HOE)
		{
			RenderMesh(meshList[GEO_WOODEN_HOE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_HOE)
		{
			RenderMesh(meshList[GEO_STONE_HOE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_HOE)
		{
			RenderMesh(meshList[GEO_GOLD_HOE_MODEL], false);
		}
		modelStack.PopMatrix();
		modelStack.PopMatrix();
		//
	...
	if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_PICKAXE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_PICKAXE || player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_PICKAXE)
	{
		Vector3 weaponoffset(1, 1, 2);
		Vector3 viewvector = (camera.target - camera.position);
		modelStack.PushMatrix();
		...
		if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_WOODEN_PICKAXE)
		{
			RenderMesh(meshList[GEO_WOODEN_PICKAXE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_STONE_PICKAXE)
		{
			RenderMesh(meshList[GEO_STONE_PICKAXE_MODEL], false);
		}
		else if (player->getItem(player->getCurrentSlot())->getID() == Item::ITEM_GOLD_PICKAXE)
		{
			RenderMesh(meshList[GEO_GOLD_PICKAXE_MODEL], false);
		}
		modelStack.PopMatrix();
		modelStack.PopMatrix();
		//
...
void SP2::RenderPassMain()
{
	...
	{
		if (m_bContinue == false)
			RenderImageToScreen(meshList[GEO_PLAYER_SELECTION], false, static_cast<float>(Application::GetWindowWidth()), static_cast<float>(Application::GetWindowHeight()), static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f), 1.f);
		else
			RenderImageToScreen(meshList[GEO_MENU], false, static_cast<float>(Application::GetWindowWidth()), static_cast<float>(Application::GetWindowHeight()), static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f), 1.f);
		RenderImageToScreen(meshList[GEO_NAVIGATOR], false, static_cast<float>(Application::GetWindowWidth() / 10), static_cast<float>(Application::GetWindowHeight() / 10), static_cast<float>(Application::GetWindowWidth()* 0.5f - 400), static_cast<float>(Application::GetWindowHeight()* 0.5f + m_fNavigatorY), 2.f);
	}
	...
	if (m_bMenu == false)
	{
		if (player->getHP() <= 0)
		{
			RenderImageToScreen(meshList[GEO_GAME_OVER], false, static_cast<float>(Application::GetWindowWidth()), static_cast<float>(Application::GetWindowHeight()), static_cast<float>(Application::GetWindowWidth()* 0.5f), static_cast<float>(Application::GetWindowHeight()* 0.5f), 1.f);
		}
		else
		{
	...
	RenderInstructions();
			...
			ss.str("");
			ss << std::to_string(int((camera.position.x + scale * 0.5) / scale)) << " " << std::to_string(int((camera.position.z + scale * 0.5) / scale));
...
void SP2::SaveAnimalData()
{
	...
	if (saveFile.is_open())
	{
		for (unsigned int i = 0; i < m_AnimalList.size(); ++i)
		{
...
void SP2::Exit()
{
	...
	for (unsigned int i = 0; i < HarvestedBushList.size(); ++i)
	{
		if (HarvestedBushList[i] != nullptr)
			delete FurnaceList[i];
	}

	for (unsigned int i = 0; i < WorldObjectPositionList.size(); ++i)
	{
